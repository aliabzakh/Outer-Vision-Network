...
OBJECTS = {
    ...
    4: {"id": 4, "color": "green", "shape": "cylinder"},   # F4 flute
    5: {"id": 5, "color": "blue", "shape": "square"},      # A4 piano
}
...
def make_assistant(reply, fail_text=False, fail_audio=False, key="test-key"):
    mock = MockOmni(reply, fail_text, fail_audio)
    music, player, voice, published = Music(CFG), FakePlayer(), FakeVoice(), []
...
class TestAssistant(unittest.TestCase):
    ...
    def test_omni_down_still_changes_the_note(self):
        ...
        self.assertEqual(music.voice_of("red", "round")["note"], "D4")               # next note up in C major
    ...
    def test_voice_failure_falls_back_to_voice_say(self):
        ...
        a, _, _, _, voice, _, sel = make_assistant(reply, fail_audio=True)
...
class TestMusic(unittest.TestCase):
    ...
    def test_lesson_uses_only_notes_on_table_and_matches_enharmonics(self):
        music = Music(CFG)
        objs = self.objs(music)                                                      # C4, F4, A4
        music.apply({"type": "set_note", "target": 4, "note": "F#4"}, objs)
        objs = self.objs(music)
        r = music.apply({"type": "start_lesson", "title": "test", "notes": ["C4", "C4", "G4", "Gb4", "A4"]}, objs)
    ...
    def test_default_teach_picks_the_most_playable_song(self):
        music = Music(CFG)
        objs = {i: {"id": i, "color": c, "shape": "round"} for i, c in enumerate(["red", "orange", "yellow"])}  # C D E
