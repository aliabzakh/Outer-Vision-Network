...
| 34 | Eye wallet limits | Limits (tip size, daily cap, contacts, expiry) are enforced by the headset's own code; the hard ceiling is the SOL the owner sends it | Enforce on-chain: a Squads smart account with spending limits, or a small program the session key must go through |
| 35 | Eye wallet link | Phone page over Web Bluetooth (needs Chrome on Android over https, self-signed in dev) or HTTP on the LAN; the link is unauthenticated because every authority change is owner-signed, but status (contacts, balance) is readable on the LAN | Native Seeker app with bonded BLE and an encrypted channel |
| 36 | Session key storage | Headset key in `wallet/session_key.json` (0600) on the laptop | Secure element / TPM on the wearable |
| 37 | Tip destinations | At most two named contacts (one per eye); amount fixed per tip | Contact list with dwell-scanning, amount picked by blink |
